import { listingsTemplateB } from "../templates/listingsHome.mjs";
...
export function setupSearch(listings) {
  ...
  searchForm.addEventListener("submit", (event) => {
    ...
    if (filteredListings.length > 0) {
      renderListingSearchTemplateHomePage(filteredListings, listingsContainer);
    } else {
...
function renderListingSearchTemplateHomePage(
  filteredListings,
  listingsContainer
) {
  listingsContainer.innerHTML = "";

  listingsContainer.append(...filteredListings.map(listingsTemplateB));
}
